...
class ADMMSolver(object):
    """ADMM solver for QP problems.
    ...
    def solve(self, example):
        ...
        try:
            # Solve using ADMM
            x_sol, y_sol, u_sol, niter = self._admm_solve(
    ...
    def _admm_solve(self, P, q, A, l, u, rho=1.0, max_iter=1000, 
                    abstol=1e-4, reltol=1e-3, verbose=False, time_limit=1000.0):
        """ADMM solver for the box-constrained QP.
        ...
        y_old = y.copy()
        y_unconstrained = None
        u_dual = np.zeros(m)  # scaled dual variable
        residual = np.zeros(m)
        residual_old = np.zeros(m)
        alpha = 1.0 * np.ones(m) # relaxation parameters
        over_relaxed_Ax = alpha * (A @ x) + (1 - alpha) * y 
        ...
        for k in range(1, max_iter + 1):
            ...
            # 1. gradually change alpha
            # delta = residual * residual_old
            # alpha[delta > 1e-6] += 0.1
            # alpha[delta < -1e-6] -= 0.1
            # alpha = np.clip(alpha, 1.0, 1.9)

            # 2. drastically change alpha
            # delta = residual * residual_old
            # alpha[delta > 1e-6] = 1.9
            # alpha[delta <= 1e-6] = 1.0

            # 3. gradually change alpha based on y clipping, doesn't quite make sense, even if sign flipped
            # if y_unconstrained is not None:
            #     mask = np.logical_or(np.abs(y_unconstrained - l) < 1e-6, np.abs(y_unconstrained - u) < 1e-6)
            #     alpha[mask] += 0.1
            #     alpha[~mask] -= 0.1
            #     alpha = np.clip(alpha, 1.0, 1.9)

            # 4. per-constraint primal feas vs. dual step size balancing
            prim_feas_error = np.clip(A @ x - y_old, a_min=1e-8, a_max=None)
            dual_step_size = np.clip(over_relaxed_Ax - y, a_min=1e-8, a_max=None)
            ratio = prim_feas_error / dual_step_size
            alpha[ratio > 2.0] += 0.1
            alpha[ratio < 0.5] -= 0.1
            alpha = np.clip(alpha, 1.0, 1.9)

            over_relaxed_Ax = alpha * (A @ x) + (1 - alpha) * y
            y_unconstrained = over_relaxed_Ax + u_dual
            y_old = y.copy()
            y = np.clip(y_unconstrained, l, u) # TODO: y is clipped, alpha larger; otherwise smaller
            
            # dual update: u_dual = u_dual + (A x - y)
            u_dual = u_dual + over_relaxed_Ax - y
